...
const ArchivesProvider = ({ children }) => {
  ...
  useEffect(() => {
    if (authState.isLoggedIn) {
      (async () => {
        try {
          const {
            data: { archives: archivesFromResponse },
          } = await receiveAllArchives(authState.authToken);
  ...
  const moveExistingNoteToArchive = async (NoteDetailsGiven) => {
    try {
      const {
        data: { notes: notesFromResponse, archives: archivesFromResponse },
      } = await transferServiceOfExistingNoteToArchive(
        NoteDetailsGiven,
        authState.authToken
      );
      setArchivedNotesList(archivesFromResponse);
  ...
  const restoreArchivedNotes = async (NoteDetailsGiven) => {
    try {
      const {
        data: { notes: notesFromResponse, archives: archivesFromResponse },
      } = await restoreArchivedNoteService(
        NoteDetailsGiven,
        authState.authToken
      );
      setNotesList(notesFromResponse);
